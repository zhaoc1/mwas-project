...
#$ -o /wynton/group/pollard/czhao/snps_out                       #-- output directory (fill in)
#$ -e /wynton/group/pollard/czhao/snps_err                       #-- error directory (fill in)
...
do
  ...
  ${GNUTIME} -v python -m midas2 run_snps \
    --sample_name ${sample_name} -1 ${lr1} -2 ${lr1} \
    --midasdb_name newdb --midasdb_dir ${midasdb_dir} \
    --fragment_length 1000 --num_cores ${TREADS} \
